...
//////////////////////////////////////////////////////////////////////////////////  
  void Adafruit_TCS34725(uint8_t it=TCS34725_INTEGRATIONTIME_50MS,tcs34725Gain_t gain=TCS34725_GAIN_4X);
 ...
 *  @param  value
 */
void write8(int8 reg, int8 value) 
{
  int8 buffer[2];
  buffer[0]=(int8)(TCS34725_COMMAND_BIT | reg);
  buffer[1]=value; 
  i2c_transfer(TCS34725_stream,TCS34725_ADDRESS<<1, buffer, 2);
}
...
{  
  int8 buffer[1];
  buffer[0]=(int8)(TCS34725_COMMAND_BIT | reg); 
  i2c_transfer(TCS34725_stream,TCS34725_ADDRESS<<1, buffer, 1, buffer, 1);
  return buffer[0];
 ...
 *  @return value
 */
int16 read16(int8 reg) 
{
  int8 buffer[2];
  buffer[0]=(int8)(TCS34725_COMMAND_BIT | reg);
  buffer[1]=0; 
  i2c_transfer(TCS34725_stream,TCS34725_ADDRESS<<1, buffer, 1, buffer, 2);
  return ((int16)(buffer[1]) << 8) | ((int16)(buffer[0]) & 0xFF);
}
 ...
 *          Gain
 */
void Adafruit_TCS34725(uint8_t it, tcs34725Gain_t gain) {
...
void clearInterrupt() {
  int8 buffer[1];
  buffer[0]=(int8)(TCS34725_COMMAND_BIT | 0x66);
  i2c_transfer(TCS34725_stream,TCS34725_ADDRESS<<1, buffer, 1);
}
